...
int xhci_get_ep_ctx_state(xhci_slot_t* slot, xhci_endpoint_t* ep) {
    ...
    xhci_cache_flush(&ep->epc->epc0, sizeof(ep->epc->epc0),
...
void xhci_set_dbcaa(xhci_t* xhci, uint32_t slot_id, zx_paddr_t paddr) {
    ...
    xhci_cache_flush(&xhci->dcbaa[slot_id], sizeof(xhci->dcbaa[slot_id]), ZX_CACHE_FLUSH_DATA);
}
...
static void xhci_handle_events(xhci_t* xhci, int interrupter) {
    ...
    // the event ring buffer is read-only so this will act only as an invalidate operation
    xhci_cache_flush(er->start, er->buffer.size, ZX_CACHE_FLUSH_DATA | ZX_CACHE_FLUSH_INVALIDATE);
