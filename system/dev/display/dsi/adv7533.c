...
static void adv7533_mainchn_write(dsi_t* dsi, uint8_t d1, uint8_t d2) {
    dsi->write_buf[0] = d1;
    dsi->write_buf[1] = d2;
    i2c_transact_sync(&dsi->i2c_dev.i2c, I2C_MAIN, dsi->write_buf, 2, NULL, 0, NULL);
}

static void adv7533_mainchn_read(dsi_t* dsi, uint8_t d1, uint8_t len) {
    size_t act_read = 0;
    dsi->write_buf[0] = d1;
    i2c_transact_sync(&dsi->i2c_dev.i2c, I2C_MAIN, dsi->write_buf, 1, dsi->write_buf, len, &act_read);
}

static void adv7533_cecchn_write(dsi_t* dsi, uint8_t d1, uint8_t d2) {
    dsi->write_buf[0] = d1;
    dsi->write_buf[1] = d2;
    i2c_transact_sync(&dsi->i2c_dev.i2c, I2C_CEC, dsi->write_buf, 2, NULL, 0, NULL);
}

static void adv7533_edidchn_read(dsi_t* dsi, uint8_t d1, uint8_t len) {
    size_t act_read = 0;
    dsi->write_buf[0] = d1;
    i2c_transact_sync(&dsi->i2c_dev.i2c, I2C_EDID, dsi->write_buf, 1, dsi->write_buf, len, &act_read);
}
