...
public:
    ...
    // as 2^|pages_pow2|).
    void InvalidateIotlbPageLocked(uint32_t domain_id, dev_vaddr_t vaddr,
                                   uint pages_pow2) TA_REQ(lock_);
...
private:
    ...
    static ds::Bdf decode_bus_txn_id(uint64_t bus_txn_id) {
        ds::Bdf bdf;
        bdf.set_bus(BITS_SHIFT(bus_txn_id, 15, 8));
        bdf.set_dev(BITS_SHIFT(bus_txn_id, 7, 3));
        bdf.set_func(BITS_SHIFT(bus_txn_id, 2, 0));
        return bdf;
    }
